...
#usage: sbatch vcf-filter-nomissing-maf05-eSNPs.sh input.vcf
...
| grep -vF './.' \
| vcffilter -f "TYPE = snp & QUAL > 30 & AF > 0.05 & AF < 0.95 & NUMALT = 1" -g "DP > 7" $1 \
| fix_freebayes_snps.py \
> $(basename $1 .vcf)_biallelic_NoNA_minMAF05_DP7.vcf

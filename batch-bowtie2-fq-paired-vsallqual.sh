...
for i in $FQ; do
    ...
    echo "srun bowtie2 --no-unal -p 3 -X 1500 --rg-id $BASE --rg SM:$BASE --very-sensitive -x $1 -1 ${BASE}_1.txt.gz -2 ${BASE}_2.txt.gz > $BASE.sam" >> TEMPBATCH.sbatch
    echo "samtools view -bS ${BASE}.sam > ${BASE}_BTVS-UNSORTED.bam " >> TEMPBATCH.sbatch
